...
static void record_tp_capacitance(enum check_data_type type, int value)
{
	...
	if (((board_id & HW_VER_MAIN_MASK) == HW_G610T10_VER) || ((board_id & HW_VER_MAIN_MASK) == HW_H30T_VER) || ((board_id & HW_VER_MAIN_MASK) == HW_H30U_VER))
		{
...
static int out_of_range(enum check_data_type type, int value)
{
	...
	else if((board_id & HW_VER_MAIN_MASK) == HW_G610T10_VER)
		{	
			switch(type)
			{
				case CY_CHK_MUT_RAW:
...
static int cyttsp4_check_raw_data(struct device *dev)
{
	...
	if(((board_id & HW_VER_MAIN_MASK) == HW_G750_VER)||((board_id & HW_VER_MAIN_MASK) == HW_G730_VER)||((board_id & HW_VER_MAIN_MASK) == HW_G730U_VER))
		{
			/* Start scan */
			rc = _cyttsp4_exec_scan_cmd(dev);
				if (rc < 0){
