...
#include "imx134sunnymipiraw_Sensor.h"
#include "imx134sunnymipiraw_Camera_Sensor_para.h"
#include "imx134sunnymipiraw_CameraCustomized.h"
...
UINT32 IMX134SUNNYMIPIOpen(void)
{
    int  retry = 0; 
	kal_uint16 sensorid;
    // check if sensor ID correct
    retry = 3; 
	SENSORDB("[IMX134MIPI]enter IMX134SUNNYMIPIOpen function\n");
    do {
	       ...
	       spin_unlock(&imx134_drv_lock);
		   if (IMX134MIPI_sensor_id == IMX134SUNNY_SENSOR_ID)
			   break; 
		   retry--; 
	    }
	while (retry > 0);
	...
	spin_unlock(&imx134_drv_lock);
	SENSORDB("[IMX134MIPI]exit IMX134SUNNYMIPIOpen function\n");
    return ERROR_NONE;
}
...
{
    int  retry = 3; 
	SENSORDB("[IMX134MIPI]enter IMX134MIPIGetSensorID(sunny) function\n");
    ...
    do {		
        ...
        if (*sensorID == IMX134SUNNY_SENSOR_ID)
            break;
        retry--; 
    } while (retry > 0);
    ...
    mt_set_gpio_pull_select(GPIO_CAMERA_ID_PIN, GPIO_PULL_UP); //save power; sunny id pin is high
	SENSORDB("[IMX134MIPI]exit IMX134MIPIGetSensorID(sunny) function\n");
    return ERROR_NONE;
}
...
*
*************************************************************************/
UINT32 IMX134SUNNYMIPIClose(void)
{
...
UINT32 IMX134SUNNYMIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    SENSORDB("[IMX134MIPI]enter IMX134SUNNYMIPIGetResolution function\n");
    ...
    SENSORDB("IMX134SUNNYMIPIGetResolution :8-14");    

    return ERROR_NONE;
}   /* IMX134MIPIGetResolution() */

UINT32 IMX134SUNNYMIPIGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
                                                MSDK_SENSOR_INFO_STRUCT *pSensorInfo,
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{ 
	SENSORDB("[IMX134MIPI]enter IMX134SUNNYMIPIGetInfo function\n");
	switch(ScenarioId){
	...
	spin_unlock(&imx134_drv_lock);
    SENSORDB("[IMX134MIPI]exit IMX134SUNNYMIPIGetInfo function\n");
    return ERROR_NONE;
}   /* IMX134MIPIGetInfo() */


UINT32 IMX134SUNNYMIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{    
		spin_lock(&imx134_drv_lock);	
		IMX134CurrentScenarioId = ScenarioId;
		spin_unlock(&imx134_drv_lock);
		SENSORDB("[IMX134MIPI]enter IMX134SUNNYMIPIControl function\n");
    switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	...
	SENSORDB("[IMX134MIPI]exit IMX134SUNNYMIPIControl function\n");
    return ERROR_NONE;
} /* IMX134MIPIControl() */
...
UINT32 IMX134SUNNYMIPIFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                pSensorDefaultData->SensorId=IMX134SUNNY_SENSOR_ID;
...
{
    IMX134SUNNYMIPIOpen,
    IMX134SUNNYMIPIGetInfo,
    IMX134SUNNYMIPIGetResolution,
    IMX134SUNNYMIPIFeatureControl,
    IMX134SUNNYMIPIControl,
    IMX134SUNNYMIPIClose
};

UINT32 IMX134SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
