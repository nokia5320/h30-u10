...
kal_uint16 BF3905MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
    ...
    return TRUE;//Fix coverity error
}
...
void BF3905MIPI_Initial_Setting(void)
{
	...
	BF3905MIPI_write_cmos_sensor(0x8a,0xa2);//96 djj mod for banding 50hz 20130607
	BF3905MIPI_write_cmos_sensor(0x8b,0x87);//7d djj mod for banding 60hz 20130607
	...
	BF3905MIPI_write_cmos_sensor(0x03,0xa0);//FOR SHADING EFFECT
	...
	BF3905MIPI_write_cmos_sensor(0x6c,0xc2);//djj add 20130607 for ic internal voltage higher
    Sleep(300);//DELAY=300
}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    return ERROR_NONE;//Fix coverity error
   ...
   switch (para)
	{
        case MEFFECT_OFF:
		{
            ...
            BF3905MIPI_write_cmos_sensor(0xb4, 0x63);//0x03 for no effect 
            BF3905MIPI_write_cmos_sensor(0x56, 0x40);
        }
            break;
...
BOOL BF3905MIPI_set_param_banding(UINT16 para)
{
	...
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
	    {
			...
			BF3905MIPI_write_cmos_sensor(0x80, 0x92);			
			BF3905MIPI_write_cmos_sensor(0x8a, 0xa2); // for bading effect 50 hz
	    }
		break;
		...
		case AE_FLICKER_MODE_60HZ:
	    {
			...
			BF3905MIPI_write_cmos_sensor(0x8b, 0x87); // for bading effect 60 hz
	    }
		break;
