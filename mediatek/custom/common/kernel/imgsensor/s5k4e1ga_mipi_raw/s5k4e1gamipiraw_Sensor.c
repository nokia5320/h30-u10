//Move driver from product directory to common directory
//Optimize 4E1 driver, and decrease mipi clock to 429Mbps
...
//vcm otp start layer
#define S5K4E1GA_LAYER_VCM_START  S5K4E1GA_LAYER_START
//vcm otp end layer
#define S5K4E1GA_LAYER_VCM_END  17
//awb otp start layer
...
#define S5K4E1GA_LAYER_VCM_NUMS  (S5K4E1GA_LAYER_VCM_END - S5K4E1GA_LAYER_VCM_START +1)

//AWB otp exist flag
#define    S5K4E1GA_OTP_AWB                  1
//VCM otp exit flag
#define    S5K4E1GA_OTP_VCM                  2
#define    S5K4E1GA_OTP_ALL  ( S5K4E1GA_OTP_AWB | S5K4E1GA_OTP_VCM)
...
static void S5K4E1GAPreviewSetting(void)
{
    ...
    //Pclcok chagne to 85.8Mhz, mipi clock to 429Mbps
...
/******************************************************************************
Function:        // S5K4E1GA_Check_OTP_Location
...
{
    kal_uint32 reg310D = 0;//reg 0x310d value
    kal_uint32 reg310E = 0;//reg 0x310e value
    kal_uint32 reg310F = 0;//reg 0x310f value


    S5K4E1GA_write_cmos_sensor(0x310c, 18);//use layer 18 to find which location OTP data lays in

    reg310D = S5K4E1GA_read_cmos_sensor(0x310d);
    reg310E = S5K4E1GA_read_cmos_sensor(0x310e);
    reg310F = S5K4E1GA_read_cmos_sensor(0x310f);

    SENSORDB("%s read layer 18: reg310D:0x%x, reg310E:0x%x, reg310F:0x%x \n", __func__, reg310D, reg310E, reg310F);
...
void S5K4E1GA_Read_Otp_Regs(struct S5K4E1GA_Otp_Struct *current_otp, kal_uint32 reg, kal_bool isMsb)
{
    ...
    SENSORDB("%s reg:0x%x, isMsb:%d ", __func__, reg, isMsb);
    ...
    for (layer = S5K4E1GA_LAYER_AWB_START; layer <= S5K4E1GA_LAYER_AWB_END; layer++)
    {
        ...
        //SENSORDB("%s regValues[%d]:0x%x", __func__, i, regValues[i]);
    }
    ...
    current_otp->iWB_RG=   (regValues[10] <<12) + (regValues[11] <<8)
                                               + (regValues[12] <<4) + regValues[13];
    current_otp->iWB_BG=   (regValues[14] <<12) + (regValues[15] <<8)
...
/******************************************************************************
Function:        // S5K4E1GA_Check_Read_VCM_Otp
...
Others:         //
******************************************************************************/
void S5K4E1GA_Check_Read_VCM_Otp(struct S5K4E1GA_Otp_Struct *current_otp)
{

    int layer =0, i = 0;
    kal_uint32 regValues[S5K4E1GA_LAYER_VCM_NUMS] = {0};
    kal_uint32 reg = 0, mask = 0;
    S5K4E1GA_OTP_Location vcmLocation = S5K4E1GA_OTP_MAX;//which position vcm lays in

    //read from right to left
    for (vcmLocation = S5K4E1GA_OTP_310D_LSB ; vcmLocation <= S5K4E1GA_OTP_310F_LSB; vcmLocation++)
    {

        switch (vcmLocation)
        {
            case S5K4E1GA_OTP_310D_LSB://reg 0x310d lsb
                reg = 0x310d;
                mask = S5K4E1GA_LSB;
                break;
            case S5K4E1GA_OTP_310E_MSB://reg 0x310e msb
                reg = 0x310e;
                mask = S5K4E1GA_MSB;
                break;
            case S5K4E1GA_OTP_310E_LSB://reg 0x310e lsb
                reg = 0x310e;
                mask = S5K4E1GA_LSB;
                break;
            case S5K4E1GA_OTP_310F_MSB://reg 0x310f msb
                reg = 0x310f;
                mask = S5K4E1GA_MSB;
                break;
            case S5K4E1GA_OTP_310F_LSB://reg 0x310f lsb
                reg = 0x310f;
                mask = S5K4E1GA_LSB;
                break;
            default:
                SENSORDB("%s Error vcmLocation %d", __func__, vcmLocation);
                return;
        }

        for (layer  = S5K4E1GA_LAYER_VCM_START; layer <= S5K4E1GA_LAYER_VCM_END; layer++)
        {
                i = layer-S5K4E1GA_LAYER_VCM_START;
                S5K4E1GA_write_cmos_sensor(0x310c, layer);//set layer
                regValues[i] = S5K4E1GA_read_cmos_sensor(reg);//read specific reg value

                if (S5K4E1GA_LSB ==mask)//LSB
                {
                    regValues[i] =  regValues[i] & S5K4E1GA_LSB;
                }
                else//MSB
                ...
                //SENSORDB("%s regValues[%d]:0x%x, reg:%x", __func__, i, regValues[i], reg);
        }

        //copy readed vcm data into otp struct according to otp map
        current_otp->iVCM_Start =  (regValues[1]<<8) + (regValues[2]<<4) + regValues[3];
        current_otp->iVCM_End = (regValues[5] <<8) + (regValues[6] <<4) + regValues[7];

        //check if VCM start and VCM end value is valid
        if (( 0 != current_otp->iVCM_Start) && (0 != current_otp->iVCM_End))
        {
            SENSORDB("%s VCM_Otp found, location:%d ", __func__, vcmLocation);
            //found vcm otp
            S5K4E1GA_otp_read_flag |= S5K4E1GA_OTP_VCM;//read sucessful
            return;
        }
        //one of VCM start or end is not zero, the value is invalid
        else if ( ((0 == current_otp->iVCM_Start) && (0 != current_otp->iVCM_End)) ||
                ((0 != current_otp->iVCM_Start) && (0 == current_otp->iVCM_End)))
        {
            SENSORDB("%s Error! Invalide VCM Otp", __func__);
            return;//error otp
        }

    }

   SENSORDB("%s Error! No VCM Otp \n", __func__);
...
bool S5K4E1GA_Read_Awb_Otp(S5K4E1GA_OTP_Location location, struct S5K4E1GA_Otp_Struct *current_otp)
{
    ...
    {
        SENSORDB("%s Error! Invalid OTP location:%d \n", __func__, location);
        return KAL_FALSE;
    }
    SENSORDB("%s location:%d \n", __func__, location);
...
/******************************************************************************
Function:        // S5K4E1GA_Update_Awb_Gain
...
{
    SENSORDB("%s,otp_ptr.iProduct_Year:0x%x",__func__,otp_ptr.iProduct_Year);
    SENSORDB("%s,otp_ptr.iProduct_Month:0x%x",__func__,otp_ptr.iProduct_Month);
    SENSORDB("%s,otp_ptr.iProduct_Date:0x%x",__func__,otp_ptr.iProduct_Date);
    SENSORDB("%s,otp_ptr.iCamera_Id:0x%x",__func__,otp_ptr.iCamera_Id);
    SENSORDB("%s,otp_ptr.iSupplier_Version_Id:0x%x",__func__,otp_ptr.iSupplier_Version_Id);
    SENSORDB("%s,otp_ptr.iVersion:0x%x\n",__func__,otp_ptr.iVersion);

    SENSORDB("%s,otp_ptr.iWB_RG:0x%x",__func__,otp_ptr.iWB_RG);
    SENSORDB("%s,otp_ptr.iWB_BG:0x%x",__func__,otp_ptr.iWB_BG);
    SENSORDB("%s,otp_ptr.iVCM_Start:0x%x",__func__,otp_ptr.iVCM_Start);
    SENSORDB("%s,otp_ptr.iVCM_End:0x%x",__func__,otp_ptr.iVCM_End);
...
bool S5K4E1GA_Update_Otp(void)
{
    ...
    S5K4E1GA_Check_Read_VCM_Otp(&current_otp);//read VCM OTP
    ...
    if ((0 == current_otp.iWB_RG) || (0 ==current_otp.iWB_BG))
    {
        SENSORDB("%s Error!Zero RG or BG data! RG:0x%x, BG:0x%x!\n", __func__, current_otp.iWB_RG, current_otp.iWB_BG);
        return KAL_FALSE;
    }
...
Others:         //
******************************************************************************/
UINT32 S5K4E1GA_Check_OTP(UINT32 *pFeatureReturnPara32)
{
    ...
    if ( S5K4E1GA_OTP_ALL == (S5K4E1GA_otp_read_flag & S5K4E1GA_OTP_ALL))
        *pFeatureReturnPara32 = 1;//opt exist
    else
        *pFeatureReturnPara32 = 0;//no OTP

    SENSORDB("S5K4E1GA_Check_OTP: S5K4E1GA_otp_read_flag:0x%x", S5K4E1GA_otp_read_flag);
    return ERROR_NONE;
}
...
UINT32 S5K4E1GAGetSensorID(UINT32 *sensorID) 
{
    ...
    if (KAL_FALSE == searchMainSensor)
    {
         SENSORDB("S5K4E1GAGetSensorID  searchMainSensor = FALSE!\n ");
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->SensorHorFOV = 56;
    pSensorInfo->SensorVerFOV = 42;
    ...
    //modify P41 for g730t camera flinker and white vertical stripe
    ...
    switch (ScenarioId)
    {
        ...
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            ...
            //modify P41 for g730t camera flinker and white vertical stripe
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 24; //14; 
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_VALIDATE_OTP:
            S5K4E1GA_Check_OTP(pFeatureReturnPara32);//check OTP exist
            break;
#endif
        default:
            break;
