...
static const kal_uint32 RG_Ratio_Typical = 0x2eb;//the average of 4 Golden samples' RG ratio
static const kal_uint32 BG_Ratio_Typical = 0x2dc;//the average of 4 Golden samples' BG ratio
...
//bit0:module info, bit1:AWB, bit2:VCM
#define    OV5648MIPI_OTP_MODULE_INFO          1
#define    OV5648MIPI_OTP_AWB                  2
#define    OV5648MIPI_OTP_VCM                  4
#define    OV5648MIPI_OTP_ALL  (OV5648MIPI_OTP_MODULE_INFO | OV5648MIPI_OTP_AWB | OV5648MIPI_OTP_VCM)
...
0x10~0x11    Bank 1:0x3d00~0x3d01    VCM
                          Group 2
0x12~0x16    Bank 1:0x3d02~0x3d06    Module info
0x17~0x1c    Bank 1:0x3d07~0x3d0c    AWB
0x1d~0x1e    Bank 1:0x3d0d~0x3d0e    VCM
******************************************************************************/
...
/******************************************************************************
Function:        // OV548MIPI_OTP_Read_Module_info
...
Others:         //
******************************************************************************/
bool OV548MIPI_OTP_Read_Module_info(struct OV5648MIPI_Otp_Struct * otp_ptr)
{
    ...
    otp_ptr->iProduct_Year = OV5648MIPI_read_cmos_sensor(0x3d02);
    otp_ptr->iProduct_Month= OV5648MIPI_read_cmos_sensor(0x3d03);
    otp_ptr->iProduct_Date = OV5648MIPI_read_cmos_sensor(0x3d04);
    otp_ptr->iCamera_Id = OV5648MIPI_read_cmos_sensor(0x3d05);
    otp_ptr->iSupplier_Version_Id = OV5648MIPI_read_cmos_sensor(0x3d06);

    SENSORDB("%s,group 2 otp_ptr.iProduct_Year:%d\n",__func__,otp_ptr->iProduct_Year);
    SENSORDB("%s,group 2 otp_ptr.iSupplier_Version_Id:%d\n",__func__,otp_ptr->iSupplier_Version_Id);


    OV5648MIPI_OTP_Clear();

    if (0 !=otp_ptr->iProduct_Year)// year is not zero, we assume module info is correct
    {
        SENSORDB("%s, Module info lays in group 2\n",__func__);
        return KAL_TRUE;
    }
    ...
    SENSORDB("%s,group 1 otp_ptr.iProduct_Year:%d\n",__func__,otp_ptr->iProduct_Year);
    SENSORDB("%s,group 1 otp_ptr.iSupplier_Version_Id:%d\n",__func__,otp_ptr->iSupplier_Version_Id);
...
/******************************************************************************
Function:        // OV5648MIPI_OTP_Read_AWB
...
Others:         //
******************************************************************************/
bool OV5648MIPI_OTP_Read_AWB(struct OV5648MIPI_Otp_Struct * otp_ptr)
{
    kal_uint32 rG = 0;
    kal_uint32 bG = 0;
    ...
    otp_ptr->iWB_RG_H = OV5648MIPI_read_cmos_sensor(0x3d07);
    otp_ptr->iWB_RG_L = OV5648MIPI_read_cmos_sensor(0x3d08);
    otp_ptr->iWB_BG_H = OV5648MIPI_read_cmos_sensor(0x3d09);
    otp_ptr->iWB_BG_L = OV5648MIPI_read_cmos_sensor(0x3d0a);
    otp_ptr->iWB_GbGr_H = OV5648MIPI_read_cmos_sensor(0x3d0b);
    otp_ptr->iWB_GbGr_L= OV5648MIPI_read_cmos_sensor(0x3d0c);
    ...
    if ((0 != rG) && (0 != bG))//rG and bG all not zero, so awb info is correct
    {
        SENSORDB("%s, AWB info lays in group 1\n",__func__);
        return KAL_TRUE;//otp read sucess
    }
    else
    {
...
Others:         //
******************************************************************************/
bool OV5648MIPI_OTP_Read_VCM(struct OV5648MIPI_Otp_Struct * otp_ptr)
{
    ...
    //VCM : Bank 1:0x3d0d~0x3d0e
    otp_ptr->iVCM_Start= OV5648MIPI_read_cmos_sensor(0x3d0d);
    otp_ptr->iVCM_End= OV5648MIPI_read_cmos_sensor(0x3d0e);

    //iVCM_Start and iVCM_End all not zero, so awb info is correct
    if ((0 != otp_ptr->iVCM_Start) && (0 != otp_ptr->iVCM_End))
    {
        OV5648MIPI_OTP_Clear();
        SENSORDB("%s, VCM info lays in group 2\n",__func__);
        return KAL_TRUE;//otp read sucess
    }
    //one of iVCM_Start and iVCM_End is not zero, so group 2 is not correct
    else if ( ((0 == otp_ptr->iVCM_Start) && (0 != otp_ptr->iVCM_End)) ||
            ((0 != otp_ptr->iVCM_Start) && (0 == otp_ptr->iVCM_End)))
    {
        OV5648MIPI_OTP_Clear();
        SENSORDB("%s,  Error !Group 2 VCM info is not correct!\n",__func__);
        return KAL_FALSE;//otp read fail
    }

    //VCM:Bank 1:0x3d00~0x3d01
    otp_ptr->iVCM_Start= OV5648MIPI_read_cmos_sensor(0x3d00);
    otp_ptr->iVCM_End= OV5648MIPI_read_cmos_sensor(0x3d01);

    OV5648MIPI_OTP_Clear();

    if ((0 != otp_ptr->iVCM_Start) && (0 != otp_ptr->iVCM_End))
    {
        SENSORDB("%s, VCM info lays in group 1\n",__func__);
        return KAL_TRUE;//otp read sucess
    }
    else
    {
...
Others:         //
******************************************************************************/
void OV5648MIPI_Read_Otp(struct OV5648MIPI_Otp_Struct * current_otp)
{
    if (KAL_TRUE == OV548MIPI_OTP_Read_Module_info(current_otp))
    {
        OV5648MIPI_otp_read_flag |= OV5648MIPI_OTP_MODULE_INFO;
    }
    if (KAL_TRUE == OV5648MIPI_OTP_Read_AWB(current_otp))
    {
        OV5648MIPI_otp_read_flag |= OV5648MIPI_OTP_AWB;
    }
    if (KAL_TRUE == OV5648MIPI_OTP_Read_VCM(current_otp))
    {
        OV5648MIPI_otp_read_flag |= OV5648MIPI_OTP_VCM;
    }

    SENSORDB("%s, OV5648MIPI_otp_read_flag:0x%x\n",__func__, OV5648MIPI_otp_read_flag);
...
bool OV5648MIPI_Update_Otp(void)
{
    ...
    OV5648MIPI_Read_Otp( &current_otp);
    ...
    if (0 ==(OV5648MIPI_otp_read_flag & OV5648MIPI_OTP_AWB))
    {
...
UINT32 OV5648MIPI_Check_OTP(UINT32 *pFeatureReturnPara32)
{
    ...
    if (OV5648MIPI_OTP_ALL == (OV5648MIPI_otp_read_flag & OV5648MIPI_OTP_ALL))
...
UINT32 OV5648GetSensorID(UINT32 *sensorID) 
{
    ...
    {
         SENSORDB("OV5648MIPIGetSensorID  searchMainSensor = FALSE!\n ");
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
