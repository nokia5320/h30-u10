...
/******************************************************************************
Function:        // CAMERA_HW_Validate_OTP_Read_Proc
Description:    //Check if the ov5647's otp eixst, the other sensor need not check.
Input:            //
Output:         //
Return:         //
Others:         //
******************************************************************************/
static int  CAMERA_HW_Validate_OTP_Read_Proc(char *page, char **start, off_t off,
                                                                                       int count, int *eof, void *data)
{
    int len = 0;
    unsigned int uValidate = 0;
    int i = 0, needCheck = 0;
    //Add OV8850 OTP check function.
    const char *Otp_checklist[]={"ov5647mipiraw"};//sensors to be check

    for (i =0; i < sizeof(Otp_checklist)/sizeof(char *); i++)
    {
        PK_DBG("CAMERA_HW_Validate_OTP_Read_Proc: i:%d, current :%s, check :%s \n", i , g_invokeSensorNameStr[0] ,Otp_checklist[i]);
        if (0 == strcmp(Otp_checklist[i], g_invokeSensorNameStr[0]))//check if this sensor need check
        {
            needCheck = 1;//this sensor need check
            break;
        }

    }
    PK_DBG("CAMERA_HW_Validate_OTP_Read_Proc: needCheck:%d \n", needCheck);

    if (needCheck)//if sensor is need check
    {
        if ( NULL != g_pSensorFunc)//if g_pSensorFunc is not NULL
        {
            //get OTP info
            g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_MAIN_SENSOR, SENSOR_FEATURE_VALIDATE_OTP, (MUINT8*)&uValidate, (MUINT32*)sizeof(uValidate));
            msleep(20);
        }
        else
        {
            PK_DBG("CAMERA_HW_Validate_OTP_Read_Proc:g_pSensorFunc is NULL, Error!\n");
        }
        PK_DBG("CAMERA_HW_Validate_OTP_Read_Proc:%s,%d\n", g_invokeSensorNameStr[0] , uValidate);
    }
    else//the othere sensor need not check
    {
        uValidate = 1;//default OK
        PK_DBG("CAMERA_HW_Validate_OTP_Read_Proc: Need not check, defalut OK\n");
    }
...
/******************************************************************************
Function:        // CAMERA_HW_Validate_OTP_Write_Proc
...
Others:         //
******************************************************************************/
static int  CAMERA_HW_Validate_OTP_Write_Proc (struct file *file, const char *buffer, unsigned long count,
                                                                     void *data)
{
    return count;
}
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    struct proc_dir_entry *prEntry_OTP;
    ...
    //Register proc file for otp info
    prEntry_OTP = create_proc_entry("driver/camsensor_otp", 0, NULL);
    if (prEntry_OTP) {  /* create success*/
        prEntry_OTP->read_proc = CAMERA_HW_Validate_OTP_Read_Proc;
        prEntry_OTP->write_proc = CAMERA_HW_Validate_OTP_Write_Proc;
    }
    else {
        PK_ERR("add /proc/driver/camsensor_otp entry fail \n");
    }
