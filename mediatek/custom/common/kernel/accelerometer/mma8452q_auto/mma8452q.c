...
#define DEBUG 1
/*----------------------------------------------------------------------------*/
#define CONFIG_MMA8452Q_LOWPASS   /*apply low pass filter on output*/       
/*----------------------------------------------------------------------------*/
...
static int mma8452q_init_flag =-1; // 0<==>OK -1 <==> fail
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
struct data_filter {
    s16 raw[C_MAX_FIR_LENGTH][MMA8452Q_AXES_NUM];
    int sum[MMA8452Q_AXES_NUM];
    int num;
    int idx;
};
/*----------------------------------------------------------------------------*/
...
struct mma8452q_i2c_data {
    ...
    atomic_t                selftest;
	atomic_t				filter;
    s16                     cali_sw[MMA8452Q_AXES_NUM+1];

    /*data*/
    s8                      offset[MMA8452Q_AXES_NUM+1];  /*+1: for 4-byte alignment*/
    s16                     data[MMA8452Q_AXES_NUM+1];

#if defined(CONFIG_MMA8452Q_LOWPASS)
    atomic_t                firlen;
    atomic_t                fir_en;
...
static void MMA8452Q_power(struct acc_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "MMA8452Q"))
			{
		...
		{
			if (!hwPowerDown(hw->power_id, "MMA8452Q"))
			{
		...
		
	{
	  // hwmsen_read_block(client, addr, buf, 0x06);
       // dumpReg(client);
		...
		data[MMA8452Q_AXIS_X] = (s16)((buf[MMA8452Q_AXIS_X*2] << 8) |
		         (buf[MMA8452Q_AXIS_X*2+1]));
		data[MMA8452Q_AXIS_Y] = (s16)((buf[MMA8452Q_AXIS_Y*2] << 8) |
		         (buf[MMA8452Q_AXIS_Y*2+1]));
		data[MMA8452Q_AXIS_Z] = (s16)((buf[MMA8452Q_AXIS_Z*2] << 8) |
		         (buf[MMA8452Q_AXIS_Z*2+1]));
		...
		data[MMA8452Q_AXIS_Z] += priv->cali_sw[MMA8452Q_AXIS_Z];
		...
		if(atomic_read(&priv->filter))
		{
			if(atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend))
			{
				...
				{                
					priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_X] = data[MMA8452Q_AXIS_X];
					priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_Y] = data[MMA8452Q_AXIS_Y];
					priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_Z] = data[MMA8452Q_AXIS_Z];
					priv->fir.sum[MMA8452Q_AXIS_X] += data[MMA8452Q_AXIS_X];
					priv->fir.sum[MMA8452Q_AXIS_Y] += data[MMA8452Q_AXIS_Y];
					priv->fir.sum[MMA8452Q_AXIS_Z] += data[MMA8452Q_AXIS_Z];
					if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
					{
						GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", priv->fir.num,
							priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_X], priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_Y], priv->fir.raw[priv->fir.num][MMA8452Q_AXIS_Z],
							priv->fir.sum[MMA8452Q_AXIS_X], priv->fir.sum[MMA8452Q_AXIS_Y], priv->fir.sum[MMA8452Q_AXIS_Z]);
					}
					priv->fir.num++;
					priv->fir.idx++;
				}
				else
				{
					...
					priv->fir.sum[MMA8452Q_AXIS_X] -= priv->fir.raw[idx][MMA8452Q_AXIS_X];
					priv->fir.sum[MMA8452Q_AXIS_Y] -= priv->fir.raw[idx][MMA8452Q_AXIS_Y];
					priv->fir.sum[MMA8452Q_AXIS_Z] -= priv->fir.raw[idx][MMA8452Q_AXIS_Z];
					priv->fir.raw[idx][MMA8452Q_AXIS_X] = data[MMA8452Q_AXIS_X];
					priv->fir.raw[idx][MMA8452Q_AXIS_Y] = data[MMA8452Q_AXIS_Y];
					priv->fir.raw[idx][MMA8452Q_AXIS_Z] = data[MMA8452Q_AXIS_Z];
					priv->fir.sum[MMA8452Q_AXIS_X] += data[MMA8452Q_AXIS_X];
					priv->fir.sum[MMA8452Q_AXIS_Y] += data[MMA8452Q_AXIS_Y];
					priv->fir.sum[MMA8452Q_AXIS_Z] += data[MMA8452Q_AXIS_Z];
					priv->fir.idx++;
					data[MMA8452Q_AXIS_X] = priv->fir.sum[MMA8452Q_AXIS_X]/firlen;
					data[MMA8452Q_AXIS_Y] = priv->fir.sum[MMA8452Q_AXIS_Y]/firlen;
					data[MMA8452Q_AXIS_Z] = priv->fir.sum[MMA8452Q_AXIS_Z]/firlen;
					if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
					{
						GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n", idx,
						priv->fir.raw[idx][MMA8452Q_AXIS_X], priv->fir.raw[idx][MMA8452Q_AXIS_Y], priv->fir.raw[idx][MMA8452Q_AXIS_Z],
						priv->fir.sum[MMA8452Q_AXIS_X], priv->fir.sum[MMA8452Q_AXIS_Y], priv->fir.sum[MMA8452Q_AXIS_Z],
						data[MMA8452Q_AXIS_X], data[MMA8452Q_AXIS_Y], data[MMA8452Q_AXIS_Z]);
					}
				}
...
/*----------------------------------------------------------------------------*/
static int MMA8452Q_ReadOffset(struct i2c_client *client, s8 ofs[MMA8452Q_AXES_NUM])
{    
	int err;
	...
	GSE_LOG("fwq read off:  offX=%x ,offY=%x ,offZ=%x\n",ofs[MMA8452Q_AXIS_X],ofs[MMA8452Q_AXIS_Y],ofs[MMA8452Q_AXIS_Z]);
...
static int MMA8452Q_ResetCalibration(struct i2c_client *client)
{
    ...
    MMA8452Q_SetPowerMode(obj->client,true);
...
{  
	...
	act[obj->cvt.map[MMA8452Q_AXIS_X]] = obj->cvt.sign[MMA8452Q_AXIS_X]*raw[MMA8452Q_AXIS_X];
	act[obj->cvt.map[MMA8452Q_AXIS_Y]] = obj->cvt.sign[MMA8452Q_AXIS_Y]*raw[MMA8452Q_AXIS_Y];
	act[obj->cvt.map[MMA8452Q_AXIS_Z]] = obj->cvt.sign[MMA8452Q_AXIS_Z]*raw[MMA8452Q_AXIS_Z];                        
...
//	int res = 0;
	...
	GSE_LOG("OLDOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n", 
		raw[MMA8452Q_AXIS_X], raw[MMA8452Q_AXIS_Y], raw[MMA8452Q_AXIS_Z],
		obj->offset[MMA8452Q_AXIS_X], obj->offset[MMA8452Q_AXIS_Y], obj->offset[MMA8452Q_AXIS_Z],
		obj->cali_sw[MMA8452Q_AXIS_X], obj->cali_sw[MMA8452Q_AXIS_Y], obj->cali_sw[MMA8452Q_AXIS_Z]);

	/*calculate the real offset expected by caller*/
	cali[MMA8452Q_AXIS_X] += dat[MMA8452Q_AXIS_X];
	cali[MMA8452Q_AXIS_Y] += dat[MMA8452Q_AXIS_Y];
	cali[MMA8452Q_AXIS_Z] += dat[MMA8452Q_AXIS_Z];

	GSE_LOG("UPDATE: (%+3d %+3d %+3d)\n", 
		dat[MMA8452Q_AXIS_X], dat[MMA8452Q_AXIS_Y], dat[MMA8452Q_AXIS_Z]);

	obj->offset[MMA8452Q_AXIS_X] = (s8)(obj->cvt.sign[MMA8452Q_AXIS_X]*(cali[obj->cvt.map[MMA8452Q_AXIS_X]])*(divisor));
	obj->offset[MMA8452Q_AXIS_Y] = (s8)(obj->cvt.sign[MMA8452Q_AXIS_Y]*(cali[obj->cvt.map[MMA8452Q_AXIS_Y]])*(divisor));
	obj->offset[MMA8452Q_AXIS_Z] = (s8)(obj->cvt.sign[MMA8452Q_AXIS_Z]*(cali[obj->cvt.map[MMA8452Q_AXIS_Z]])*(divisor));

	/*convert software calibration using standard calibration*/
	obj->cali_sw[MMA8452Q_AXIS_X] =0; //obj->cvt.sign[MMA8452Q_AXIS_X]*(cali[obj->cvt.map[MMA8452Q_AXIS_X]])%(divisor);
	obj->cali_sw[MMA8452Q_AXIS_Y] =0; //obj->cvt.sign[MMA8452Q_AXIS_Y]*(cali[obj->cvt.map[MMA8452Q_AXIS_Y]])%(divisor);
	obj->cali_sw[MMA8452Q_AXIS_Z] =0;// obj->cvt.sign[MMA8452Q_AXIS_Z]*(cali[obj->cvt.map[MMA8452Q_AXIS_Z]])%(divisor);

	GSE_LOG("NEWOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n", 
		obj->offset[MMA8452Q_AXIS_X] + obj->cali_sw[MMA8452Q_AXIS_X], 
		obj->offset[MMA8452Q_AXIS_Y] + obj->cali_sw[MMA8452Q_AXIS_Y], 
		obj->offset[MMA8452Q_AXIS_Z] + obj->cali_sw[MMA8452Q_AXIS_Z], 
		obj->offset[MMA8452Q_AXIS_X], obj->offset[MMA8452Q_AXIS_Y], obj->offset[MMA8452Q_AXIS_Z],
		obj->cali_sw[MMA8452Q_AXIS_X], obj->cali_sw[MMA8452Q_AXIS_Y], obj->cali_sw[MMA8452Q_AXIS_Z]);
	//
	...
	MMA8452Q_SetPowerMode(obj->client,false);
	msleep(20);
	if(err = hwmsen_write_byte(obj->client, MMA8452Q_REG_OFSX, obj->offset[MMA8452Q_AXIS_X]))
	{
		GSE_ERR("write offset fail: %d\n", err);
		return err;
	}
    msleep(20);
	hwmsen_read_byte_sr(obj->client,MMA8452Q_REG_OFSX,&testdata);
	GSE_LOG("write offsetX: %x\n", testdata);
	
	if(err = hwmsen_write_byte(obj->client, MMA8452Q_REG_OFSY, obj->offset[MMA8452Q_AXIS_Y]))
	{
		GSE_ERR("write offset fail: %d\n", err);
		return err;
	}
	msleep(20);
	hwmsen_read_byte_sr(obj->client,MMA8452Q_REG_OFSY,&testdata);
	GSE_LOG("write offsetY: %x\n", testdata);
	
	if(err = hwmsen_write_byte(obj->client, MMA8452Q_REG_OFSZ, obj->offset[MMA8452Q_AXIS_Z]))
	{
		GSE_ERR("write offset fail: %d\n", err);
		return err;
	}
	msleep(20);
	hwmsen_read_byte_sr(obj->client,MMA8452Q_REG_OFSZ,&testdata);
...
/*----------------------------------------------------------------------------*/
static int MMA8452Q_CheckDeviceID(struct i2c_client *client)
{
	u8 databuf[10];    
	int res = 0;
...
//	struct mma8452q_i2c_data *obj = i2c_get_clientdata(client);
	...
	memset(databuf, 0, sizeof(u8)*10);    
	databuf[0] = MMA8452Q_REG_XYZ_DATA_CFG;    
...
/*----------------------------------------------------------------------------*/
static int MMA8452Q_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	u8 databuf[10];    
	int res = 0;
	...
	if(hwmsen_read_byte_sr(client, MMA8452Q_REG_CTL_REG1, databuf))
	{
		GSE_ERR("read power ctl register err!\n");
		return MMA8452Q_ERR_I2C;
	}
	GSE_LOG("fwq read MMA8452Q_REG_CTL_REG1 =%x in %s \n",databuf[0],__FUNCTION__);
...
static int MMA8452Q_SetIntEnable(struct i2c_client *client, u8 intenable)
{
	...
	memset(databuf, 0, sizeof(u8)*10);    
	databuf[0] = MMA8452Q_REG_CTL_REG4;    
	databuf[1] = intenable;
...
/*//we do not use interrupt
	res = MMA8452Q_SetIntEnable(client, MMA8452Q_DATA_READY);        
	if(res != MMA8452Q_SUCCESS)//0x2E->0x80
	{
		return res;
	}
*/
...
/*----------------------------------------------------------------------------*/
static int MMA8452Q_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	u8 databuf[10];    
...
static int MMA8452Q_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		obj->data[MMA8452Q_AXIS_X] += obj->cali_sw[MMA8452Q_AXIS_X];
		obj->data[MMA8452Q_AXIS_Y] += obj->cali_sw[MMA8452Q_AXIS_Y];
		obj->data[MMA8452Q_AXIS_Z] += obj->cali_sw[MMA8452Q_AXIS_Z];
		
		/*remap coordinate*/
		acc[obj->cvt.map[MMA8452Q_AXIS_X]] = obj->cvt.sign[MMA8452Q_AXIS_X]*obj->data[MMA8452Q_AXIS_X];
		acc[obj->cvt.map[MMA8452Q_AXIS_Y]] = obj->cvt.sign[MMA8452Q_AXIS_Y]*obj->data[MMA8452Q_AXIS_Y];
		acc[obj->cvt.map[MMA8452Q_AXIS_Z]] = obj->cvt.sign[MMA8452Q_AXIS_Z]*obj->data[MMA8452Q_AXIS_Z];

		//GSE_LOG("Mapped gsensor data: %d, %d, %d!\n", acc[MMA8452Q_AXIS_X], acc[MMA8452Q_AXIS_Y], acc[MMA8452Q_AXIS_Z]);

		//Out put the mg
		acc[MMA8452Q_AXIS_X] = acc[MMA8452Q_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MMA8452Q_AXIS_Y] = acc[MMA8452Q_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MMA8452Q_AXIS_Z] = acc[MMA8452Q_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;		
		

		sprintf(buf, "%04x %04x %04x", acc[MMA8452Q_AXIS_X], acc[MMA8452Q_AXIS_Y], acc[MMA8452Q_AXIS_Z]);
...
static int MMA8452Q_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	{
		sprintf(buf, "%04x %04x %04x", obj->data[MMA8452Q_AXIS_X], 
			obj->data[MMA8452Q_AXIS_Y], obj->data[MMA8452Q_AXIS_Z]);
...
/*
	...
	*/
	...
	res = MMA8452Q_SetDataFormat(client, MMA8452Q_RANGE_2G);
	if(res != MMA8452Q_SUCCESS) //0x2C->BW=100Hz
	{
		return res;
	}
	res = MMA8452Q_SetDataResolution(client, MMA8452Q_12BIT_RES);
	...
	//set self test reg
	memset(databuf, 0, sizeof(u8)*10);    
	databuf[0] = MMA8452Q_REG_CTL_REG2;//set self test    
	if(hwmsen_read_byte_sr(client, MMA8452Q_REG_CTL_REG2, databuf))
	{
		GSE_ERR("read power ctl register err!\n");
		return MMA8452Q_ERR_I2C;
...
/*----------------------------------------------------------------------------*/
static int MMA8452Q_JudgeTestResult(struct i2c_client *client, s32 prv[MMA8452Q_AXES_NUM], s32 nxt[MMA8452Q_AXES_NUM])
{
    struct criteria {
        int min;
        int max;
	...
	else if((tmp_resolution&MMA8452Q_12BIT_RES) && (detectRage&MMA8452Q_RANGE_4G))
	{
		...
		ptr = &self[3];
	else if(detectRage&MMA8452Q_RANGE_4G)//8 bit resolution
		ptr = &self[4];
	else if(detectRage&MMA8452Q_RANGE_8G)//8 bit resolution
		ptr = &self[5];
    ...
    if (((nxt[MMA8452Q_AXIS_X] - prv[MMA8452Q_AXIS_X]) > (*ptr)[MMA8452Q_AXIS_X].max) ||
        ((nxt[MMA8452Q_AXIS_X] - prv[MMA8452Q_AXIS_X]) < (*ptr)[MMA8452Q_AXIS_X].min)) {
        GSE_ERR("X is over range\n");
        res = -EINVAL;
    }
    if (((nxt[MMA8452Q_AXIS_Y] - prv[MMA8452Q_AXIS_Y]) > (*ptr)[MMA8452Q_AXIS_Y].max) ||
        ((nxt[MMA8452Q_AXIS_Y] - prv[MMA8452Q_AXIS_Y]) < (*ptr)[MMA8452Q_AXIS_Y].min)) {
        GSE_ERR("Y is over range\n");
        res = -EINVAL;
    }
    if (((nxt[MMA8452Q_AXIS_Z] - prv[MMA8452Q_AXIS_Z]) > (*ptr)[MMA8452Q_AXIS_Z].max) ||
        ((nxt[MMA8452Q_AXIS_Z] - prv[MMA8452Q_AXIS_Z]) < (*ptr)[MMA8452Q_AXIS_Z].min)) {
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	MMA8452Q_ReadChipInfo(client, strbuf, MMA8452Q_BUFSIZE);
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	MMA8452Q_ReadSensorData(client, strbuf, MMA8452Q_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);            
}
/*----------------------------------------------------------------------------*/
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	{    
		...
		len += snprintf(buf+len, PAGE_SIZE-len, "[ALL]    (%+3d, %+3d, %+3d) : (%+3d, %+3d, %+3d)\n", 
			obj->offset[MMA8452Q_AXIS_X]*mul + obj->cali_sw[MMA8452Q_AXIS_X],
			obj->offset[MMA8452Q_AXIS_Y]*mul + obj->cali_sw[MMA8452Q_AXIS_Y],
			obj->offset[MMA8452Q_AXIS_Z]*mul + obj->cali_sw[MMA8452Q_AXIS_Z],
			tmp[MMA8452Q_AXIS_X], tmp[MMA8452Q_AXIS_Y], tmp[MMA8452Q_AXIS_Z]);
...
{   /*write anything to this register will trigger the process*/
	...
	for(idx = 0; idx < num; idx++)
	{
		...
		avg_prv[MMA8452Q_AXIS_X] += prv[idx].raw[MMA8452Q_AXIS_X];
		avg_prv[MMA8452Q_AXIS_Y] += prv[idx].raw[MMA8452Q_AXIS_Y];
		avg_prv[MMA8452Q_AXIS_Z] += prv[idx].raw[MMA8452Q_AXIS_Z];        
		GSE_LOG("[%5d %5d %5d]\n", prv[idx].raw[MMA8452Q_AXIS_X], prv[idx].raw[MMA8452Q_AXIS_Y], prv[idx].raw[MMA8452Q_AXIS_Z]);
	}
	
	avg_prv[MMA8452Q_AXIS_X] /= num;
	avg_prv[MMA8452Q_AXIS_Y] /= num;
	avg_prv[MMA8452Q_AXIS_Z] /= num; 
	...
	MMA8452Q_InitSelfTest(client);
	GSE_LOG("SELFTEST:\n");  
/*
	MMA8452Q_ReadData(client, nxt[0].raw);
	GSE_LOG("nxt[0].raw[MMA8452Q_AXIS_X]: %d\n", nxt[0].raw[MMA8452Q_AXIS_X]);
	GSE_LOG("nxt[0].raw[MMA8452Q_AXIS_Y]: %d\n", nxt[0].raw[MMA8452Q_AXIS_Y]);
	GSE_LOG("nxt[0].raw[MMA8452Q_AXIS_Z]: %d\n", nxt[0].raw[MMA8452Q_AXIS_Z]);
	*/
	for(idx = 0; idx < num; idx++)
	{
		...
		avg_nxt[MMA8452Q_AXIS_X] += nxt[idx].raw[MMA8452Q_AXIS_X];
		avg_nxt[MMA8452Q_AXIS_Y] += nxt[idx].raw[MMA8452Q_AXIS_Y];
		avg_nxt[MMA8452Q_AXIS_Z] += nxt[idx].raw[MMA8452Q_AXIS_Z];        
		GSE_LOG("[%5d %5d %5d]\n", nxt[idx].raw[MMA8452Q_AXIS_X], nxt[idx].raw[MMA8452Q_AXIS_Y], nxt[idx].raw[MMA8452Q_AXIS_Z]);
	}
	...
	// 
	MMA8452Q_Init(client, 0);

	avg_nxt[MMA8452Q_AXIS_X] /= num;
	avg_nxt[MMA8452Q_AXIS_Y] /= num;
	avg_nxt[MMA8452Q_AXIS_Z] /= num;    

	GSE_LOG("X: %5d - %5d = %5d \n", avg_nxt[MMA8452Q_AXIS_X], avg_prv[MMA8452Q_AXIS_X], avg_nxt[MMA8452Q_AXIS_X] - avg_prv[MMA8452Q_AXIS_X]);
	GSE_LOG("Y: %5d - %5d = %5d \n", avg_nxt[MMA8452Q_AXIS_Y], avg_prv[MMA8452Q_AXIS_Y], avg_nxt[MMA8452Q_AXIS_Y] - avg_prv[MMA8452Q_AXIS_Y]);
	GSE_LOG("Z: %5d - %5d = %5d \n", avg_nxt[MMA8452Q_AXIS_Z], avg_prv[MMA8452Q_AXIS_Z], avg_nxt[MMA8452Q_AXIS_Z] - avg_prv[MMA8452Q_AXIS_Z]); 

	if(!MMA8452Q_JudgeTestResult(client, avg_prv, avg_nxt))
	{
...
static ssize_t show_firlen_value(struct device_driver *ddri, char *buf)
{
	...
	if(atomic_read(&obj->firlen))
	{
		...
		{
			GSE_LOG("[%5d %5d %5d]\n", obj->fir.raw[idx][MMA8452Q_AXIS_X], obj->fir.raw[idx][MMA8452Q_AXIS_Y], obj->fir.raw[idx][MMA8452Q_AXIS_Z]);
		}
		
		GSE_LOG("sum = [%5d %5d %5d]\n", obj->fir.sum[MMA8452Q_AXIS_X], obj->fir.sum[MMA8452Q_AXIS_Y], obj->fir.sum[MMA8452Q_AXIS_Z]);
		GSE_LOG("avg = [%5d %5d %5d]\n", obj->fir.sum[MMA8452Q_AXIS_X]/len, obj->fir.sum[MMA8452Q_AXIS_Y]/len, obj->fir.sum[MMA8452Q_AXIS_Z]/len);
	}
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				if(err != MMA8452Q_SUCCESS ) //0x2C->BW=100Hz
				{
					GSE_ERR("Set delay parameter error!\n");
				}
				...
				{					
					priv->fir.num = 0;
					priv->fir.idx = 0;
					priv->fir.sum[MMA8452Q_AXIS_X] = 0;
					priv->fir.sum[MMA8452Q_AXIS_Y] = 0;
					priv->fir.sum[MMA8452Q_AXIS_Z] = 0;
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = MMA8452Q_SetPowerMode( priv->client, !sensor_power);
				}
			}
			break;
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				MMA8452Q_ReadSensorData(priv->client, buff, MMA8452Q_BUFSIZE);
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			{   
				...
				cali[MMA8452Q_AXIS_X] = sensor_data.x * obj->reso->sensitivity / GRAVITY_EARTH_1000;
				cali[MMA8452Q_AXIS_Y] = sensor_data.y * obj->reso->sensitivity / GRAVITY_EARTH_1000;
				cali[MMA8452Q_AXIS_Z] = sensor_data.z * obj->reso->sensitivity / GRAVITY_EARTH_1000;			  
				err = MMA8452Q_WriteCalibration(client, cali);			 
			}
			break;

		case GSENSOR_IOCTL_CLR_CALI:
			//GSE_LOG("fwq GSENSOR_IOCTL_CLR_CALI!!\n");
			err = MMA8452Q_ResetCalibration(client);
			break;
		...
		case GSENSOR_IOCTL_GET_CALI:
			...
			sensor_data.x = cali[MMA8452Q_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
			sensor_data.y = cali[MMA8452Q_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
			sensor_data.z = cali[MMA8452Q_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static int mma8452q_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		//read old data
		if ((err = hwmsen_read_byte_sr(client, MMA8452Q_REG_CTL_REG1, &dat))) 
		{
...
static void mma8452q_early_suspend(struct early_suspend *h) 
{
	...
	atomic_set(&obj->suspend, 1); 
	/*
	if(err = hwmsen_write_byte(obj->client, MMA8452Q_REG_POWER_CTL, 0x00))
	{
		GSE_ERR("write power control fail!!\n");
		return;
	}  
