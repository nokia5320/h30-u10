...
static int kx023_remove(struct platform_device *pdev);
...
static int kx023_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = kx023_operate;
...
static int kx023_i2c_remove(struct i2c_client *client)
{
	...
	if((err = kx023_delete_attr(&kx023_gsensor_driver.driver)))
	{
...
/*----------------------------------------------------------------------------*/
static int kx023_probe(struct platform_device *pdev) 
{
	struct acc_hw *hw = kx023_get_cust_acc_hw();
	GSE_FUN();

	KX023_power(hw, 1);
	//kx023_force[0] = hw->i2c_num;
