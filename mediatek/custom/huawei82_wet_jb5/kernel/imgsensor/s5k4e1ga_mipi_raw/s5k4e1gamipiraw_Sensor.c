...
static void S5K4E1GAPreviewSetting(void)
{
    ...
    //Pclcok change to 85.8Mhz, mipi clock to 429Mbps
...
bool S5K4E1GA_Update_Otp(void)
{
    ...
    gG_gain = G_gain;
    gB_gain = B_gain;
    S5K4E1GA_otp_read_flag = OTP_VALID;
    spin_unlock(&s5k4e1gamipiraw_drv_lock);
...
UINT32 S5K4E1GA_Check_OTP(UINT32 *pFeatureReturnPara32)
{
    ...
    if ( OTP_VALID == S5K4E1GA_otp_read_flag )
...
UINT32 S5K4E1GAGetSensorID(UINT32 *sensorID) 
{
    ...
    {
         SENSORDB("S5K4E1GAGetSensorID  searchMainSensor = KAL_TRUE!\n ");
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
