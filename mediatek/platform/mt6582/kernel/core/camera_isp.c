/******************************************************************************
 * camera_isp.c - MT6582 Linux ISP Device Driver
 *
...
#define ISP_REG_ADDR_CTL_CROP_Y         (ISP_ADDR + 0x114)
#define ISP_REG_ADDR_CTL_CROP_X_DATA        (0x1FFF1FFF) 
// CAM_CTL_CROP_X   (0x4110)  bit[12:0]                  CSR_CAM_MDP_XSTART
//CAM_CTL_CROP_X   (0x4110)  bit[28:16]    CSR_CAM_MDP_XEND
#define ISP_REG_ADDR_CTL_CROP_Y_DATA        (0x1FFF1FFF) 
// CAM_CTL_CROP_Y   (0x4114)  bit[12:0]                  CSR_CAM_MDP_YSTART
//CAM_CTL_CROP_Y   (0x4114)  bit[28:16]    CSR_CAM_MDP_YEND
#if 0
1.	MDP_YSTART = 0 , MDP_YEND = 0
2.	MDP_XSTART = 0, MDP_XEND = 0  (要使用 read modify write，因為 CAM_CTL_CROP_X 中有其他的register)
3.	Wait cmq idle (原本的code即可)
4.	結束
#endif
...
static int ISPResetPass2(int params)
{
    MINT32 i = 0;
    MINT32 Ret = 0;
    MINT32 m32RegCropY= 0;
    MINT32 m32RegCropX= 0;
    LOG_DBG("ISPResetPass2_v02 >>");
...
//    LOG_DBG("CQ STATUS: bef");
    ...
    LOG_DBG("ISP_REG_ADDR_CTL_CROP_Y>: 0x%08x ", ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_Y));
    m32RegCropY= ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_Y);
    ISP_WR32((void *)ISP_REG_ADDR_CTL_CROP_Y, m32RegCropY&(~ISP_REG_ADDR_CTL_CROP_Y_DATA));
    LOG_DBG("ISP_REG_ADDR_CTL_CROP_Y>>: 0x%08x ", ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_Y));
    ...
    ISP_WR32((void *)ISP_REG_ADDR_CTL_CROP_X, m32RegCropX&(~ISP_REG_ADDR_CTL_CROP_X_DATA));
//    LOG_DBG("ISP_REG_ADDR_CTL_CROP_X>>: 0x%08x ", ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_X));
    ...
    LOG_DBG("i - %d ", i);
//    ISP_WR32((void *)ISP_REG_ADDR_CTL_CROP_Y, m32RegCropY);
    LOG_DBG("ISP_REG_ADDR_CTL_CROP_Y>>>: 0x%08x ", ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_Y));
...
static MVOID ISP_EnableClock(MBOOL En)
{
    ...
    if(En)  // enable clock.
    {
        enable_clock(MT_CG_DISP0_SMI_COMMON,"CAMERA"); //new for MT6582, confirm with Abrams
...
//            LOG_DBG(" <");
    ...
    else    // disable clock.
    {
        ...
        disable_clock(MT_CG_DISP0_SMI_COMMON,"CAMERA"); //new for MT6582, confirm with Abrams
        g_EnableClkCnt--;
...
#endif
//    LOG_DBG("ISPDbgPortDump() - TEST");
//    ISPDbgPortDump((int)i); //test function
//    ISPResetPass2((int)i); //test function
    LOG_DBG("-");
}
            ...
            case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE :                
                if(copy_from_user(&g_rt_buf_info, (void*)rt_buf_ctrl.data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0)
                {
                    ...
                    if((reg_val & 0x01) || (reg_val2 & 0x01)) 
                    {
                        ...
                        //if(g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) 
                        //{
                            LOG_DBG("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),2O(0x%x),fps(%d)us",rt_dma,
                                                                                                 g_rt_buf_info.base_pAddr,
                                                                                                 ISP_RD32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR),
                                                                                                 ISP_RD32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR),
                                                                                                 g_avg_frame_time);
            ...
            case ISP_RT_BUF_CTRL_DEQUE :               
                ...
                if((reg_val & 0x01) || (reg_val2 & 0x01))
                {
                    ...
                    for(i = 0; i < g_deque_buf.count; i++)
                    {
                        ...
                        //if(g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) 
                        //{
                            LOG_DBG("[rtbc][DEQUE]:TStamp\"%d.%06d\",co(0x%08x),c2o(0x%08x),i(%d),iBuf+i(%d),VA(0x%x),PA(0x%x),O(0x%x),2O(0x%x)", \
...
//    if (request_irq(CAMERA_ISP_IRQ0_ID, (irq_handler_t)ISP_Irq, IRQF_TRIGGER_HIGH, "isp", NULL))
    {
        LOG_ERR("MT6582_CAM_IRQ_LINE IRQ LINE NOT AVAILABLE!!");
        goto EXIT;
    }
    ...
    MVOID *pData)
{
    ...
    p += sprintf(p, " MT6582 ISP Register\n");
    p += sprintf(p, "====== top ====\n");
