...
static int general_clk_enable_op(struct cg_clk *clk)
{
    ...
    if(!strcmp(clk->grp->name, "CG_MFG"))
    {   
    	trace_printk("[VR power] MFG_CG ON\n");
	}
    else if(!strcmp(clk->grp->name, "CG_IMAGE"))
	{
        switch(clk->mask){
        case 0x0001:
            trace_printk("[VR power] LARB2_SMI_CG ON\n");
            break;
        case 0x0020:
        	trace_printk("[VR power] CAM_SMI_CG ON\n");
            break;
        case 0x0040:
        	trace_printk("[VR power] CAM_CAM_CG ON\n");
            break;
        case 0x0080:
        	trace_printk("[VR power] SEN_TG_CG ON\n");
            break;
        case 0x0100:
        	trace_printk("[VR power] SEN_CAM_CG ON\n");
            break;
        case 0x0200:
        	trace_printk("[VR power] VENC_JPENC_CG ON\n");
            break;
        default:
        	break;    
        }                    
    }
	else if(!strcmp(clk->grp->name, "CG_DISP0"))
    {
        switch(clk->mask){
//        case 0x0002:
//        	clk_info("[%s]: SMI_LARB0 on\n", __func__);
//        	break;
        case 0x0200:
            trace_printk("[VR power] MDP_TDSHP_CG ON\n");
            break;
        case 0x0400:
            trace_printk("[VR power] MDP_WROT_CG ON\n");
            break;
        case 0x0800:
            trace_printk("[VR power] MDP_WDMA_CG ON\n");
            break;
        case 0x1000:
            trace_printk("[VR power] MDP_RSZ1_CG ON\n");
            break;
        case 0x2000:
            trace_printk("[VR power] MDP_RSZ0_CG ON\n");
            break;                
        case 0x4000:
            trace_printk("[VR power] MDP_RDMA_CG ON\n");
            break;
        case 0x10000:
            trace_printk("[VR power] CAM_MDP_CG ON\n");
            break;
        default:
        	break;
        }
...
static int general_clk_disable_op(struct cg_clk *clk)
{
    ...
    if(!strcmp(clk->grp->name, "CG_MFG"))
    {   
    	trace_printk("[VR power] MFG_CG OFF\n");
        //printk("[VR power] MFG_CG OFF\n");
	}
    else if(!strcmp(clk->grp->name, "CG_IMAGE"))
	{
        switch(clk->mask){
        case 0x0001:
            trace_printk("[VR power] LARB2_SMI_CG OFF\n");
            break;
        case 0x0020:
        	trace_printk("[VR power] CAM_SMI_CG OFF\n");
            break;
        case 0x0040:
        	trace_printk("[VR power] CAM_CAM_CG OFF\n");
            break;
        case 0x0080:
        	trace_printk("[VR power] SEN_TG_CG OFF\n");
            break;
        case 0x0100:
        	trace_printk("[VR power] SEN_CAM_CG OFF\n");
            break;
        case 0x0200:
        	trace_printk("[VR power] VENC_JPENC_CG OFF\n");
            break;
        default:
        	break;    
        }
	...
	else if(!strcmp(clk->grp->name, "CG_DISP0"))
    {
        switch(clk->mask){
//        case 0x0002:
//            clk_info("[%s]: SMI_LARB0 off\n", __func__);
//            break
        case 0x0200:
            trace_printk("[VR power] MDP_TDSHP_CG OFF\n");
            break;
        case 0x0400:
            trace_printk("[VR power] MDP_WROT_CG OFF\n");
            break;
        case 0x0800:
            trace_printk("[VR power] MDP_WDMA_CG OFF\n");
            break;
        case 0x1000:
            trace_printk("[VR power] MDP_RSZ1_CG OFF\n");
            break;
        case 0x2000:
            trace_printk("[VR power] MDP_RSZ0_CG OFF\n");
            break;                
        case 0x4000:
            trace_printk("[VR power] MDP_RDMA_CG OFF\n");
            break;
        case 0x10000:
            trace_printk("[VR power] CAM_MDP_CG OFF\n");
            break;
        default:
        	break;
        }    	
