...
#define CONNSYS_START (PMEM_MM_START + PMEM_MM_SIZE)
#define CONNSYS_SIZE (RESERVED_MEM_SIZE_FOR_CONNSYS)

#define TEST_3D_START  (CONNSYS_START + CONNSYS_SIZE)
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    phys_addr_t max_avail_addr = 0;
    phys_addr_t modem_start_addr = 0;
    phys_addr_t hole_start_addr = 0;
    resource_size_t hole_size = 0;
    ...
    if(tags->hdr.tag == ATAG_NONE) {
        for (i = 0; i < nr_modem; i++) {
            ...
            if (modem_size_list[i]) {
                printk(KERN_ALERT"fixup for modem [%d], size = 0x%llx\n", i,
                        (unsigned long long)modem_size_list[i]);
            } else {
                printk(KERN_ALERT"[Error]skip empty modem [%d]\n", i);
                continue;
            }
            printk(KERN_ALERT
                    "reserved_mem_bank_tag start = 0x%llx, "
                    "reserved_mem_bank_tag size = 0x%llx, "
                    "TOTAL_RESERVED_MEM_SIZE = 0x%llx\n",
                    (unsigned long long)reserved_mem_bank_tag->u.mem.start,
                    (unsigned long long)reserved_mem_bank_tag->u.mem.size,
                    (unsigned long long)TOTAL_RESERVED_MEM_SIZE);
            ...
            if (modem_start_addr < reserved_mem_bank_tag->u.mem.start) {
                printk(KERN_ALERT"[Error]skip modem [%d] fixup: "
                        "modem crosses memory bank boundary: 0x%llx, "
                        "reserved_mem_bank_tag->u.mem.start: 0x%llx\n", i,
                        (unsigned long long)modem_start_addr,
                        (unsigned long long)reserved_mem_bank_tag->u.mem.start);
                continue;
            }
            ...
            printk(KERN_ALERT
                    "max_avail_addr = 0x%llx, "
                    "modem_start_addr_list[%d] = 0x%llx, "
                    "hole_start_addr = 0x%llx, hole_size = 0x%llx\n",
                    (unsigned long long)max_avail_addr, i, 
                    (unsigned long long)modem_start_addr,
                    (unsigned long long)hole_start_addr, 
                    (unsigned long long)hole_size);
            MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
                    "[PHY layout]MD       :   0x%llx - 0x%llx  (0x%llx)\n",
                    (unsigned long long)modem_start_addr,
                    (unsigned long long)(modem_start_addr + modem_size_list[i] - 1),
                    (unsigned long long)modem_size_list[i]);
            /* shrink reserved_mem_bank */
            reserved_mem_bank_tag->u.mem.size -=
                (max_avail_addr - modem_start_addr);
            printk(KERN_ALERT
                    "reserved_mem_bank: start = 0x%llx, size = 0x%llx\n",
                    (unsigned long long)reserved_mem_bank_tag->u.mem.start,
                    (unsigned long long)reserved_mem_bank_tag->u.mem.size);
...
unsigned long get_CONNSYS_start(void)
{
    return (unsigned long)CONNSYS_START;
}
EXPORT_SYMBOL(get_CONNSYS_start);

unsigned long get_CONNSYS_size(void)
{
    return (unsigned long)CONNSYS_SIZE;
}
EXPORT_SYMBOL(get_CONNSYS_size);

/*
 * high memory size should be 0 or more than 8MiB
 */
//extern unsigned long totalhigh_pages;
//static void __init highmemory_size_check(void)
//{
//    if (likely(!totalhigh_pages || (totalhigh_pages << PAGE_SHIFT) >= SZ_8M))  {
//        printk(KERN_ALERT"highmemory_size_check PASS\n");
//    } else {
//        printk(KERN_ALERT"Error: high memory size (0x%08lx) less than 0x%08lx\n", 
//                (totalhigh_pages << PAGE_SHIFT),
//                (unsigned long)SZ_8M);
//        BUG();
//    }
//}
//
//late_initcall(highmemory_size_check);
