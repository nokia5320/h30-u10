...
irqreturn_t mt_ptp_isr(int irq, void *dev_id)
{
    ...
    if (PTPINTSTS == 0x1) // PTP init1 or init2
    {
        ...
        else if ((temp_ptpen & 0x7) == 0x5) // PTP init2
        {
            ...
            ptp_notice("ptp_volt_0 = 0x%x\n", ptp_volt_0);
            ptp_notice("ptp_volt_1 = 0x%x\n", ptp_volt_1);
            ptp_notice("ptp_volt_2 = 0x%x\n", ptp_volt_2);
            ptp_notice("ptp_volt_3 = 0x%x\n", ptp_volt_3);
            ptp_notice("ptp_volt_4 = 0x%x\n", ptp_volt_4);
            ptp_notice("ptp_volt_5 = 0x%x\n", ptp_volt_5);
            ptp_notice("ptp_volt_6 = 0x%x\n", ptp_volt_6);
            ptp_notice("ptp_volt_7 = 0x%x\n", ptp_volt_7);
            ptp_notice("ptp_level  = 0x%x\n", ptp_level);
        ...
        {
            ptp_notice("====================================================\n");
            ptp_notice("PTP error_0 (0x%x) : PTPINTSTS = 0x%x\n", temp_ptpen, PTPINTSTS);
            ptp_notice("PTP_SMSTATE0 (0x%x) = 0x%x\n", PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
            ptp_notice("PTP_SMSTATE1 (0x%x) = 0x%x\n", PTP_SMSTATE1, ptp_read(PTP_SMSTATE1));
            ptp_notice("====================================================\n");
    ...
    else if ((PTPINTSTS & 0x00ff0000) != 0x0)  // PTP Monitor mode
    {
        ...
        else
        {
            ...
            ptp_notice("ptp_volt_7 = 0x%x\n", ptp_volt_7);
            ptp_notice("ptp_level = 0x%x\n", ptp_level);
            ptp_notice("ISR : TEMPSPARE1 = 0x%x\n", ptp_read(TEMPSPARE1));
    ...
    else // PTP error handler
    {
        ...
        {
            ptp_notice("====================================================\n");
            ptp_notice("PTP error_1 error_2 (0x%x) : PTPINTSTS = 0x%x\n", temp_ptpen, PTPINTSTS);
            ptp_notice("PTP_SMSTATE0 (0x%x) = 0x%x\n", PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
            ptp_notice("PTP_SMSTATE1 (0x%x) = 0x%x\n", PTP_SMSTATE1, ptp_read(PTP_SMSTATE1));
            ptp_notice("====================================================\n");
        ...
        {
            ptp_notice("====================================================\n");
            ptp_notice("PTP error_m (0x%x) : PTPINTSTS = 0x%x\n", temp_ptpen, PTPINTSTS);
            ptp_notice("PTP_SMSTATE0 (0x%x) = 0x%x\n", PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
            ptp_notice("PTP_SMSTATE1 (0x%x) = 0x%x\n", PTP_SMSTATE1, ptp_read(PTP_SMSTATE1));
            ptp_notice("PTP_TEMP (0x%x) = 0x%x\n", PTP_TEMP, ptp_read(PTP_TEMP) );
            ptp_notice("PTP_TEMPMSR0 (0x%x) = 0x%x\n", PTP_TEMPMSR0, ptp_read(PTP_TEMPMSR0));
            ptp_notice("PTP_TEMPMSR1 (0x%x) = 0x%x\n", PTP_TEMPMSR1, ptp_read(PTP_TEMPMSR1));
            ptp_notice("PTP_TEMPMSR2 (0x%x) = 0x%x\n", PTP_TEMPMSR2, ptp_read(PTP_TEMPMSR2));
            ptp_notice("PTP_TEMPMONCTL0 (0x%x) = 0x%x\n", PTP_TEMPMONCTL0, ptp_read(PTP_TEMPMONCTL0));
            ptp_notice("PTP_TEMPMSRCTL1 (0x%x) = 0x%x\n", PTP_TEMPMSRCTL1, ptp_read(PTP_TEMPMSRCTL1));
            ptp_notice("====================================================\n");
