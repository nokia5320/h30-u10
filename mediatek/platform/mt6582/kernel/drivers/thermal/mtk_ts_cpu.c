...
static void thermal_reset_and_initial(void)
{
	...
	THERMAL_WRAP_WR32(0x000003FF, TEMPMONCTL1);    // counting unit is 1024 * 20ns


    THERMAL_WRAP_WR32(0x03FF0000, TEMPMONCTL2);	// sensing interval is 1024 * 20ns = 20ms


	//THERMAL_WRAP_WR32(0x0000000F, TEMPAHBPOLL);    // polling interval to check if temperature sense is ready, 32k*16
	THERMAL_WRAP_WR32(0x00FFFFFF, TEMPAHBPOLL);		//total update time = 0x1000000x20ns +  20ms
...
//	THERMAL_WRAP_WR32(0x0000008FC, TEMPHTHRE);     // set hot threshold
...
//	THERMAL_WRAP_WR32(0x00000CE4, TEMPCTHRE);      // set cold threshold
	...
	THERMAL_WRAP_WR32(0x0000000, TEMPMSRCTL0);     // temperature sampling control, one sample
    ...
    temp &=~(0x000000C0);//TSCON0[7:6]=2'b00
    ...
    THERMAL_WRAP_WR32(0x00000003, TEMPMONCTL0);                    // enable periodoc temperature sensing point 1
...
static void thermal_cal_prepare(void)
{
	...
	temp0 = DRV_Reg32(0xF0206100);
	temp1 = DRV_Reg32(0xF0206104);
	//temp2 = DRV_Reg32(0xF0009108);
	printk("[Power/CPU_Thermal] [Thermal calibration] Reg(0xF0206100)=0x%x, Reg(0xF0206104)=0x%x\n", temp0, temp1);
	...
	{
		mtktscpu_dprintk("This sample is not Thermal calibrated\n");
		g_adc_ge_t = 512;
		g_adc_oe_t = 512;
		g_degc_cali = 40;
		g_o_slope = 0;
	...
	printk("[Power/CPU_Thermal] [Thermal calibration] g_adc_ge_t      = 0x%x\n",g_adc_ge_t);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_adc_oe_t      = 0x%x\n",g_adc_oe_t);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_degc_cali     = 0x%x\n",g_degc_cali);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_adc_cali_en_t = 0x%x\n",g_adc_cali_en_t);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_o_slope       = 0x%x\n",g_o_slope);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_o_slope_sign  = 0x%x\n",g_o_slope_sign);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_id            = 0x%x\n",g_id);


	printk("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsmcu1     = 0x%x\n",g_o_vtsmcu1);
	printk("[Power/CPU_Thermal] [Thermal calibration] g_o_vtsmcu2     = 0x%x\n",g_o_vtsmcu2);
...
static void thermal_cal_prepare_2(kal_uint32 ret)
{
    ...
    printk("[Power/CPU_Thermal] [Thermal calibration] g_ge       = 0x%x\n",g_ge);
    printk("[Power/CPU_Thermal] [Thermal calibration] g_gain     = 0x%x\n",g_gain);
    printk("[Power/CPU_Thermal] [Thermal calibration] g_gain     = 0x%x\n",g_gain);
    printk("[Power/CPU_Thermal] [Thermal calibration] g_x_roomt1 = 0x%x\n",g_x_roomt1);
    printk("[Power/CPU_Thermal] [Thermal calibration] g_x_roomt2 = 0x%x\n",g_x_roomt2);
}
