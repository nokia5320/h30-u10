...
#ifndef HUAWEI_4V2_BATTERY_CHARGE
	upmu_set_rg_vbat_ov_vth(0x2);   //VBAT_OV_VTH, 4.4V,
#else
    upmu_set_rg_vbat_ov_vth(0x1);   //VBAT_OV_VTH, 4.3V,
#endif

    //if at + nboot, cancel baton check
    if(BR_TOOL_BY_PASS_PWK == g_boot_reason)
    {
        battery_xlog_printk(BAT_LOG_CRTI, "boot reason is(%d). disable baton check\n", g_boot_reason);
        upmu_set_rg_baton_en(0);        //BATON_DIS
    }
    else
    {
...
/******************************************************************************
Function:       trim_cv_voltage
  Description:    trim target cv according to calibrated cv
  Input:          target_vol -- target voltage that will be setted
  Output:         null
  Return:         trimed voltage for target voltage
  Others:         null
******************************************************************************/
static kal_uint32 trim_cv_voltage(kal_uint32 target_vol)
{
    //calibration for cv stored in efuse_dout_48_63 bit[12:8]
    //bit8 ~ bit12
    #define CALIBRATE_CV_MASK    (0x1f00)
    #define CALIBRATE_CV_SHIFT    (8)
    #define CV_MAX_TOLERANCE    (50000)  //50mV

    kal_uint32 efuse_dout = 0;
    kal_int32 reg_cali_for_4v2 = 0;
    kal_int32 cali_vol_for_4v2 = 0;
    kal_int32 vol_offset = 0;
    kal_int32 target_vol_cali = 0;

    //just trim for 4.2v or 4.35V
    if((target_vol != BATTERY_VOLT_04_200000_V) && (target_vol != BATTERY_VOLT_04_350000_V))
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[trim_cv_voltage] target-cv(%d) is not 4.2V or 4.35V, do not trim\n", target_vol);
        return target_vol;
    }

    //get calibrated cv for 4.2V
    efuse_dout = upmu_get_rg_efuse_dout_48_63();
    reg_cali_for_4v2 = (efuse_dout & CALIBRATE_CV_MASK) >> CALIBRATE_CV_SHIFT;
    cali_vol_for_4v2 = charging_value_to_parameter(VBAT_CV_VTH, GETARRAYNUM(VBAT_CV_VTH), reg_cali_for_4v2);

    vol_offset = BATTERY_VOLT_04_200000_V - cali_vol_for_4v2;
    //exceed max tolerance, do not trim
    if(CV_MAX_TOLERANCE < abs(vol_offset))
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[trim_cv_voltage] voltage offset(%d) exceed max tolerance(%d), do not trim\n", vol_offset, CV_MAX_TOLERANCE);
        return target_vol;
    }

    //only trim higher error
    if(vol_offset <= 0)
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[trim_cv_voltage] voltage offset(%d<=0), do not trim\n", vol_offset);
        return target_vol;
    }

    //for 4.2V, just use calibrated cv_vth
    if(BATTERY_VOLT_04_200000_V == target_vol)
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[trim_cv_voltage] just return calibrated value for 4.2V (%d)\n", cali_vol_for_4v2);
        return cali_vol_for_4v2;
    }

    //trim for 4.35V, choosing between BATTERY_VOLT_04_300000_V and BATTERY_VOLT_04_325000_V
    target_vol_cali = target_vol - vol_offset;
    if(BATTERY_VOLT_04_325000_V <= target_vol_cali)
    {
        target_vol_cali = BATTERY_VOLT_04_325000_V;
    }
    else
    {
        target_vol_cali =  BATTERY_VOLT_04_300000_V;
    }
    battery_xlog_printk(BAT_LOG_CRTI, "[trim_cv_voltage] voltage-offset(%d) target-voltage(%d) target-voltage_cali(%d)\n",
        vol_offset, target_vol, target_vol_cali);

    return target_vol_cali;
}
 ...
 static kal_uint32 charging_set_cv_voltage(void *data)
 {
    ...
    kal_uint32 cv_vol = trim_cv_voltage(*(kal_uint32 *)(data));

    register_value = charging_parameter_to_value(VBAT_CV_VTH, GETARRAYNUM(VBAT_CV_VTH) , cv_vol);
