...
/**********************************************************
  ...
  *
  *********************************************************/
#define fan5405_SLAVE_ADDR_WRITE   0xD6  //D6
#define fan5405_SLAVE_ADDR_Read    0xD7  //D7
...
kal_uint32 fan5405_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
   ...
   battery_xlog_printk(BAT_LOG_FULL,"--------------------------------------------------\n");
	...
	battery_xlog_printk(BAT_LOG_FULL,"[fan5405_read_interface] Reg[%x]=0x%x\n", RegNum, fan5405_reg);
	
    fan5405_reg &= (MASK << SHIFT);
    *val = (fan5405_reg >> SHIFT);
	
	battery_xlog_printk(BAT_LOG_FULL,"[fan5405_read_interface] val=0x%x\n", *val);
...
kal_uint32 fan5405_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = fan5405_read_byte(RegNum, &fan5405_reg);
    battery_xlog_printk(BAT_LOG_FULL,"[fan5405_config_interface] Reg[%x]=0x%x\n", RegNum, fan5405_reg);
    
    fan5405_reg &= ~(MASK << SHIFT);
    fan5405_reg |= (val << SHIFT);

	if(RegNum == fan5405_CON4 && val == 1 && MASK ==CON4_RESET_MASK && SHIFT == CON4_RESET_SHIFT)
	{
		// RESET bit
	}
    ...
    battery_xlog_printk(BAT_LOG_FULL,"[fan5405_config_interface] write Reg[%x]=0x%x\n", RegNum, fan5405_reg);
...
static ssize_t store_fan5405_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] \n");
    
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] write fan5405 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] read fan5405 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_fan5405);
            battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] Please use \"cat fan5405_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"******** fan5405_user_space_probe!! ********\n" );
