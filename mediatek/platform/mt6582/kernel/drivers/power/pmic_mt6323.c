...
void dct_pmic_VCAMD_sel(kal_uint32 volt)
{
    ...
    else if(volt == VOL_1350){ upmu_set_rg_vcamd_vosel(1);}
	...
	if ((board_id & HW_VER_MAIN_MASK) == HW_G610T10_VER)
		{
			...
			ret=pmic_config_interface(0x0532, 0xd, 0xffff, 8);
			xlog_printk(ANDROID_LOG_INFO,"Power/PMIC","[pmic_mt6323_init] mtk6582 increase VGP2 for debugging ");
		}
	else
		{
			//increasing VGP3 to 1.86, please help to measure it from HW
			ret=pmic_config_interface(0x0534, 0xd, 0xf, 8);
			xlog_printk(ANDROID_LOG_INFO,"Power/PMIC","[pmic_mt6323_init] mtk6582 increase VGP3 for debugging ");
		}
