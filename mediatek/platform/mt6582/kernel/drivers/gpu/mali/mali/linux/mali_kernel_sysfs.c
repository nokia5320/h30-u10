...
static int proc_curfreq_show(struct seq_file *m, void *v)
{
    //seq_printf(m, "%u\n", mt_gpufreq_cur_freq());
    seq_printf(m, "0\n");
...
static int proc_curfreq_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_curfreq_show, NULL);
}

static const struct file_operations proc_curfreq_operations = {
    .open    = proc_curfreq_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release,
};
...
/// Robin:TODO, this is only a temp solution for low-power measurement
...
static bool mali_early_suspend_register_flag = false;
/******************************************
* [Temp solution for Low-power measurement]
  read Register Early Suspend state
*******************************************/
...
static struct early_suspend mali_EarlySuspend = 
{	
    .level   = EARLY_SUSPEND_LEVEL_DISABLE_FB + 200,
	.suspend = mali_early_suspend_handler,
	.resume	 = mali_late_resume_handler,
};
...
static int mali_early_suspend_debug_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;

    if (mali_early_suspend_register_flag)
        p += sprintf(p, "mali early suspend/late resume enabled\n");
    else
        p += sprintf(p, "mali early suspend/late resume disable\n");
...
static ssize_t mali_early_suspend_register_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    int debug = 0;
    ...
    if (sscanf(buffer, "%d", &debug) == 1)
    {
        if (debug == 0) 
        {            
            if (true == mali_early_suspend_register_flag)
            {                  
               unregister_early_suspend(&mali_EarlySuspend);  
               mali_early_suspend_register_flag = false;             
               MALI_DEBUG_PRINT(1, ("[%s] suspend state= %d\n", __FUNCTION__, mali_early_suspend_register_flag));
            }              
            return count;
        }
        else if (debug == 1)
        {          
            if (false == mali_early_suspend_register_flag)  
            {                 
               register_early_suspend(&mali_EarlySuspend);
               mali_early_suspend_register_flag = true;
               MALI_DEBUG_PRINT(1, ("[%s] suspend state = %d\n", __FUNCTION__, mali_early_suspend_register_flag)); 
            }            
            return count;
        }
        ...
        {
            MALI_DEBUG_PRINT(1, ("bad argument!! should be 0 or 1 [0: disable, 1: enable]\n"));
        }
    }
    else
    {
...
static void proc_mali_unregister(void)
{
    ...
    remove_proc_entry("earlysuspend", mali_pentry);
