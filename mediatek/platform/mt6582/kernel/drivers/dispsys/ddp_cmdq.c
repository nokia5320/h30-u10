                                     ...
                                     void    *pData)
{
    ...
    *pEOF   = 0x1;
    ...
    for (index = 0; ((index < CMDQ_MAX_TASK_COUNT) && (0 == isFull)); index++)
    {
...
static void cmdq_dump_task_usage()
{
    int32_t    index;
    TaskStruct *pTask;
...
#if 0
    if (blockSize >= CMDQ_MAX_BLOCK_SIZE)
    {
        CMDQ_AEE("CMDQ", "Unsupported command size\n");
        return NULL;
    }
#endif // 0
...
static void cmdq_disable_clock(uint32_t engineFlag)
{
    EngineStruct *pEngine;
    int32_t      loopCount;
    ...
    if (engineFlag & (0x1 << tWDMA1))
    {
           ...
           clock_is_on(MT_CG_DISP0_MDP_WDMA))
        {
            ...
            {
                CMDQ_AEE("MDP", "Reset WDMA engine failed\n");
                return;
            }
    ...
    if (engineFlag & (0x1 << tWROT))
    {
           ...
           clock_is_on(MT_CG_DISP0_MDP_WROT))
        {
            ...
            if (loopCount >= CMDQ_MAX_LOOP_COUNT)
            {
                CMDQ_AEE("MDP", "Reset WROT engine failed\n");
                return;
            }
    ...
    if (engineFlag & (0x1 << tRDMA0))
    {
           ...
           clock_is_on(MT_CG_DISP0_MDP_RDMA))
        {
            ...
            if (loopCount >= CMDQ_MAX_LOOP_COUNT)
            {
                CMDQ_AEE("MDP", "Reset RDMA engine failed\n");
                return;
            }
...
static void cmdq_reset_hw_engine(int32_t engineFlag)
{
    EngineStruct *pEngine;
    int32_t      loopCount;
                                   ...
                                   int32_t    thread)
{
    ...
    value[0] = DISP_REG_GET(DISP_REG_CMDQ_THRx_PC(thread));
    value[1] = DISP_REG_GET(DISP_REG_CMDQ_THRx_END_ADDR(thread));
    value[2] = DISP_REG_GET(DISP_REG_CMDQ_THRx_WAIT_EVENTS0(thread));
    value[3] = DISP_REG_GET(DISP_REG_CMDQ_THRx_EXEC_CMDS_CNT(thread)) & 0x0FFFF;

    pThread = &(gCmdqContext.thread[thread]);

    printk(KERN_DEBUG "Thread PC: 0x%08x, End: 0x%08x, Wait Event: 0x%02x, Curr Cookie: %d, Wait Cookie: %d, Next Cookie: %d, Task Count %d\n",
        value[0], value[1], value[2], value[3], pThread->waitCookie, pThread->nextCookie, pThread->taskCount);
    ...
    if (NULL != pTask)
    {
        ...
        if (engineFlag & (0x01 << tTDSHP))
        {
            ...
            for (inner = 0; inner < 1024; inner += 1)
            {
                pError[index].regVal[tTDSHP][index] = DISP_REG_GET(0xF4006000 + (index << 2));
            }
        #endif // 0
        }
        ...
        if (engineFlag & (0x01 << tWROT))
        {
            ...
            value[9] = DISP_REG_GET(0xF40050D0);
            ...
            printk(KERN_DEBUG "[CMDQ] ROT_DMA_PERF: 0x%08x, ROT_STRIDE: 0x%08x, ROT_IN_SIZE: 0x%08x\n", 
                value[6], value[7], value[8]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_1: 0x%08x, ROT_DBUGG_2: 0x%08x, ROT_DEBUBG_3: 0x%08x\n", 
                value[9], value[10], value[11]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_4: 0x%08x, ROT_DBUGG_5: 0x%08x, ROT_DEBUBG_6: 0x%08x\n", 
                value[12], value[13], value[14]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_7: 0x%08x, ROT_DBUGG_8: 0x%08x, ROT_DEBUBG_9: 0x%08x\n", 
                value[15], value[16], value[17]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_A: 0x%08x, ROT_DBUGG_B: 0x%08x, ROT_DEBUBG_C: 0x%08x\n", 
                value[18], value[19], value[20]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_D: 0x%08x, ROT_DBUGG_E: 0x%08x, ROT_DEBUBG_F: 0x%08x\n", 
                value[21], value[22], value[23]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_10: 0x%08x, ROT_DBUGG_11: 0x%08x, ROT_DEBUBG_12: 0x%08x\n", 
                value[24], value[25], value[26]);
            printk(KERN_DEBUG "[CMDQ] ROT_DBUGG_13: 0x%08x, ROT_DBUGG_14: 0x%08x\n", 
                value[27], value[28]);
        }
                                   ...
                                   int32_t    thread)
{
    ...
    else if (0 > waitQ)  // Task be killed
    {
        ...
        {           
            ...
            DISP_REG_SET(DISP_REG_CMDQ_THRx_PC(thread), pTask->MVABase + pTask->blockSize - 16);

            cmdq_reset_hw_engine(pTask->engineFlag);
        ...
        if (TASK_STATE_DONE != pTask->taskState)
        {
            ...
            if((currPC >= pTask->MVABase) && (currPC <= (pTask->MVABase + pTask->blockSize)))
            {
                ...
                if((insts[3] & 0x08000000) != 0) // Polling some register
                {
                    ...
                    {
                        CMDQ_AEE("ISP", "Polling ISP reg timeout, inst = 0x%08x, 0x%08x\n", insts[2], insts[3]);
                    }
                    else  //22 bit == 0 -> MDP
                    {
                        CMDQ_AEE("MDP", "Polling MDP reg timeout, inst = 0x%08x, 0x%08x\n", insts[2], insts[3]);
                    }
                }
                else if((insts[3] & 0x20000000) != 0)  // Waiting some event
                {
                    ...
                    {
                        CMDQ_AEE("ISP", "Wait ISP event timeout, inst = 0x%08x, 0x%08x\n", insts[2], insts[3]);
                    }
                    else
                    {
                        CMDQ_AEE("MDP", "Wait MDP event timeout, inst = 0x%08x, 0x%08x\n", insts[2], insts[3]);
                    }
                }
                else
                {
                    CMDQ_AEE("CMDQ", "Wait MM command queue timeout, inst = 0x%08x, 0x%08x\n", insts[2], insts[3]);
                }
            }
            else
            {
                CMDQ_AEE("CMDQ", "MM command queue driver internal state error\n");
            }
...
static void cmdq_release_task(TaskStruct *pTask)
{
        ...
        (TASK_STATE_BUSY  != pTask->taskState))
    {
        ...
        {
            dma_free_coherent(NULL, pTask->blockSize + CMDQ_EXTRA_MARGIN, pTask->pVABase, pTask->MVABase);
            kfree(pTask);
        }
    }
...
void cmdqHandleError(int32_t index)
{
    ...
    if (NULL != pThread->pCurTask[(cookie + 1) % CMDQ_MAX_TASK_COUNT])
    {
        CMGQ_GET_CURRENT_TIME(pThread->pCurTask[inner]->gotIRQ);
        
        pThread->pCurTask[(cookie + 1) % CMDQ_MAX_TASK_COUNT]->taskState = TASK_STATE_ERROR;
        pThread->pCurTask[(cookie + 1) % CMDQ_MAX_TASK_COUNT] = NULL;
        pThread->taskCount--;
    }
